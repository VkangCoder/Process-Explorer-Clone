...
export function PerformanceTab({ process, themeMode }: Props) {
    ...
    useEffect(() => {
        ...
        setPoints((prev) => {
            const next = [...prev, { time: new Date().toLocaleTimeString(), cpu: process.cpu }];
    ...
    return (
        <div style={{ padding: 8 }}>
            <h4 style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <ProcessIcon executablePath={process.executablePath} /> {process.name} (PID {process.pid}) - CPU
                realtime
            </h4>
            <ResponsiveContainer width="100%" height={200}>
                <LineChart data={points}>
                    <CartesianGrid strokeDasharray="3 3" stroke={themeMode === "dark" ? "#444" : "#ccc"} />
                    <XAxis
                        dataKey="time"
                        tick={{
                            fontSize: 10,
